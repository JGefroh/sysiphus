...
if [ "$(uname)" == "Darwin" ]; then
    ...
    get_cpu_idle() { top -bn 5 | head -n 4 | tail -1 | awk '{ print $7}' | sed 's/%//'; }
else
    get_cpu_idle() { echo '-1'; }
fi
...
contact()
{
  curl -H "Content-Type: application/json" -X POST -d "{\"id\": 1000, \"disk_used_in_bytes\": $1, \"disk_free_in_bytes\": $2, \"cpu_idle\": $3}" $HOST/api/performance_update
}
